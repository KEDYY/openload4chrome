...
window.addEventListener('load', function () {
  ...
  var openload_button = document.getElementById('openload');
  var saveas_button = document.getElementById('saveas');
  var history_list = document.getElementById('history');
  var directory_list = document.getElementById('select_folder_list');
  chrome.tabs.getSelected(function (tab) {
    console.log(tab.url);
    if (isSupportHost(tab.url)) {
      url_input.value = tab.url;
      var headers = "";
      headers += "User-Agent: {ua}".format({ua: navigator.userAgent});
      headers += "\r\n" + "referer: {referrer};".format({referrer: tab.url});
  ...
  function remoteUpload() {
    ...
    if (!isValid(link)) {
      notification.notify("Please enter the download link blank!");
      return;
    }
    if (!isLink(link.value.toLowerCase())) {
      notification.notify("The download link is invalid!");
      return;
    }

    function success(result) {
      notification.notify(result.msg);
    }

    addRemoteUpload(link.value, folder.values, headers.values, success);
  }
  ...
  getFolder(null, function (result) {
    ...
    folders.forEach(function (folder) {
      directory_list.options.add(new Option(folder.name, folder.id))
    })
  });
